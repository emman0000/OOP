...
using namespace std;
class Course{
    string courseId; string courseName; int creditHours;
public: 
...
class Student {
    string StudentId; string name; int maxcourses; Course** courses; int numCourses;

public:
Student(string id , string n, int max,): StudentId(id), name(n), maxcourses(max), numCourses(0){courses = new Course*[maxcourses]; 
for(int i=0; i<maxcourses; i++){
    courses[i]=nullptr//initialising all pointers to null
}}//end constructor
...
~Student (){
 ...
 void enroll (Course* course){
    ...
    else{ cout<<"Cannot enroll into anymore courses kitna parho gae bhai?"<<endl;
 } }//end function 
...
for(int i=0; i<numcourses; i++){
    if(courses[i] == course) {
                ...
                courses[numCourses - 1] = nullptr; // Set the last slot to null
                numCourses--;
